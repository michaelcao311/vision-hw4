...
image make_integral_image(image im)
{
    ...
    for (int k = 0; k < im.c; k++) {
        for (int i = 0; i < im.w; i++) {
            int pixel_pos = i + k*im.w*im.h;
            integ.data[pixel_pos] = im.data[pixel_pos];
        }
        for (int j = 0; j < im.h; j++) {
            int pixel_pos = j*im.w + k*im.w*im.h;
            integ.data[pixel_pos] = im.data[pixel_pos];
        }
        for (int j = 1; j < im.h; j++) {
            for (int i = 1; i < im.w; i++) {
                int pos_1 = (i-1) + (j-1)*im.w + k*im.w*im.h;
                int pos_2 = (i-1) + j*im.w + k*im.w*im.h;
                int pos_3 = i + (j-1)*im.w + k*im.w*im.h;
                int pos_4 = i + j*im.w + k*im.w*im.h;
                integ.data[pos_4] = integ.data[pos_2] + integ.data[pos_3] - integ.data[pos_1] + im.data[pos_4];
            }
        }
    }
    return integ;
...
image box_filter_image(image im, int s)
{
    ...
    for (int i = 0; i < im.w; i++) {
        for (int j = 0; j < im.h; j++) {
            for (int k = 0; k < im.c; k++) {
                float bottom_right = get_pixel(integ, i+shift, j+shift, k);
                float top_right = get_pixel(integ, i+shift, j-shift-1, k);
                float bottom_left = get_pixel(integ, i-shift-1, j+shift, k);
                float top_left = get_pixel(integ, i-shift-1, j-shift-1, k);
                set_pixel(S, i, j, k, (bottom_right+top_left-top_right-bottom_left)/(s*s));
            }
        }
    }
    return S;
...
{
    int converted = 0;
    image T = make_image(im.w, im.h, 5);
    if(im.c == 3){
        converted = 1;
    ...
    for (int i = 0; i < im.w; i++) {
        for (int j = 0; j < im.h; j++) {
            ...
            float t = get_pixel(It, i, j, 0);
            set_pixel(T, i, j, 0, x*x);
            set_pixel(T, i, j, 1, y*y);
            set_pixel(T, i, j, 2, x*y);
            set_pixel(T, i, j, 3, x*t);
            set_pixel(T, i, j, 4, y*t);
        }
    }

    image S = box_filter_image(T, s);
...
image velocity_image(image S, int stride)
{
    ...
    for(j = (stride-1)/2; j < S.h; j += stride){
        for(i = (stride-1)/2; i < S.w; i += stride){
            ...
            float vx;
            float vy;
            float epsilon = 1.0/33554432;

            if (fabsf(Ixx*Iyy - Ixy*Ixy) < epsilon) {
                vx = vy = 0;
            } else {
                M.data[0][0] = Ixx;
                M.data[0][1] = Ixy;
